...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/TUPXParams"
    else
        PARAMS_DIR="$HOME/.tenup-params"
    fi
fi
...
Failed to fetch the TenUp zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
TenUp - fetch-params.sh

This script will fetch the TenUp zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common TenUp zkSNARK parameters. Note that it is
