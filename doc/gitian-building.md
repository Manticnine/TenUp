...
*Setup instructions for a gitian build of TenUp Core using a VM or physical system.*
...
to the TenUp GitHub release page.
...
- [Building TenUp Core](#building-tenup-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/tenup-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/tenup-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/tenup-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/tenup-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/tenup-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/tenup-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/tenup-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@tenup.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/tenup-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/tenup-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/tenup-osx-*-build.assert
```
