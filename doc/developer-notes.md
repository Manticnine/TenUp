...
- [Development guidelines](#development-guidelines)
    - [General TenUp Core](#general-tenup-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
TenUp Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
TenUp Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/tenup-project/tenup/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_tenup
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_tenup --log_level=test_suite
$ valgrind -v --leak-check=full src/tenupd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_tenup.coverage/index.html`.
```
...
- BitcoinMiner : Generates TUPs (if wallet is enabled).
...
be sent upstream but bugfixes may also be prudent to PR against TenUp Core so that they can be integrated
...
file descriptors it will cause problems with TenUp's `select()` loop, because
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `tenup-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `tenup-cli` and the GUI debug console use this table to determine how to
