...
UniValue spendzerocoin(const UniValue& params, bool fHelp)
{
    ...
    if (fMintChange && Params().NetworkID() != CBaseChainParams::REGTEST)
        throw JSONRPCError(RPC_WALLET_ERROR, "zTUP minting is DISABLED, cannot mint change");
...
UniValue spendzerocoinmints(const UniValue& params, bool fHelp)
{
    ...
    return DoZtupSpend(nAmount, false, true, vMintsSelected, address_str);
}
...
UniValue spendrawzerocoin(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 4 || params.size() > 5)
        throw std::runtime_error(
            ...
            "5. \"address\"          (string, optional) TenUp address to spend to. If not specified, spend to change add.\n"
...
UniValue clearspendcache(const UniValue& params, bool fHelp)
{
    if(fHelp || params.size() != 0)
        throw std::runtime_error(
            "clearspendcache\n"
            "\nClear the pre-computed zTUP spend cache, and database.\n" +
            HelpRequiringPassphrase() + "\n"
...

    {
        ...
        while (nTries < 100) {
            ...
            if (fLocked) {
                if (ztupTracker->ClearSpendCache()) {
