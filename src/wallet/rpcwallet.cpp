...
UniValue getaccountaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            ...
            "\nDEPRECATED. Returns the current TENUP address for receiving payments to this account.\n"
...
UniValue spendzerocoin(const UniValue& params, bool fHelp)
{
    ...
    const bool fMintChange = params[1].get_bool();       // Mint change to zTUP
    ...
    if (Params().NetworkID() != CBaseChainParams::REGTEST) {
        if (fMintChange)
            throw JSONRPCError(RPC_WALLET_ERROR, "zTUP minting is DISABLED (except for regtest), cannot mint change");

        if (!isPublicSpend)
            throw JSONRPCError(RPC_WALLET_ERROR, "zTUP old spend only available in regtest for tests purposes");
    }

    std::vector<CZerocoinMint> vMintsSelected;
    return DoZtupSpend(nAmount, fMintChange, fMinimizeChange, vMintsSelected, address_str, isPublicSpend);
}
...
UniValue spendzerocoinmints(const UniValue& params, bool fHelp)
{
    ...
    return DoZtupSpend(nAmount, false, true, vMintsSelected, address_str, isPublicSpend);
}


extern UniValue DoZtupSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, std::vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend)
{
...
UniValue createautomintaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
                ...
                "\nResult\n"
                "\"address\"     (string) TENUP address for auto minting\n" +
...
UniValue spendrawzerocoin(const UniValue& params, bool fHelp)
{
    ...
    return DoZtupSpend(mint.GetDenominationAsAmount(), false, true, vMintsSelected, address_str, isPublicSpend);
}
