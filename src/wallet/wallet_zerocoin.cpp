...
void CWallet::doZTupRescan(const CBlockIndex* pindex, const CBlock& block,
        std::set<uint256>& setAddedToWallet, const Consensus::Params& consensus, bool fCheckZTUP)
{
    //If this is a zapwallettx, need to read ztup
    if (fCheckZTUP && consensus.NetworkUpgradeActive(pindex->nHeight, Consensus::UPGRADE_ZC)) {
...
bool CWallet::CreateZTUPOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint)
{
    ...
    zwallet->GenerateDeterministicZTUP(denomination, coin, dMint);
        ...
        CTxDestination * changeAddress)
{
    ...
    const int nMaxSpends = Params().GetConsensus().ZC_MaxPublicSpendsPerTx; // Maximum possible spends for one zTUP public spend transaction
...
void CWallet::setZWallet(CzTUPWallet* zwallet)
{
    ...
    ztupTracker = std::unique_ptr<CzTUPTracker>(new CzTUPTracker(this));
}
