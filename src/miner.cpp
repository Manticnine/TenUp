...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake, std::vector<COutput>* availableCoins)
{
...

    {
        ...
        while (!vecPriority.empty()) {
            ...
            // zTUP check to not include duplicated serials in the same block.
...
void BitcoinMiner(CWallet* pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        LogPrintf("fProofOfStake: %s\n", fProofOfStake);
        CBlockIndex* pindexPrev = GetChainTip();
        if (!pindexPrev) {
            MilliSleep(nSpacingMillis);       // sleep a block
            continue;
        }
        if (fProofOfStake == true) {
        ...
        } else if (consensus.NetworkUpgradeActive(pindexPrev->nHeight<6 ? pindexPrev->nHeight : pindexPrev->nHeight - 6, Consensus::UPGRADE_POS)) {
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("TUPXMiner exception");
    } catch (...) {
        LogPrintf("TUPXMiner exception");
    }

    LogPrintf("TUPXMiner exiting\n");
}
