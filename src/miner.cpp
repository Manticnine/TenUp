        ...
        if (fProofOfStake) {
            ...
            if (pblock->IsZerocoinStake()) {
                ...
                if (!SignBlockWithKey(*pblock, key)) {
                    LogPrintf("%s: Signing new block with zTUP key failed \n", __func__);
                    return NULL;
                }
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        if (pblock->IsZerocoinStake()) {
            ...
            pwalletMain->ztupTracker->ListMints(true, true, true); //update the state
        }
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("TenUpMiner exception");
    } catch (...) {
        LogPrintf("TenUpMiner exception");
    }

    LogPrintf("TenUpMiner exiting\n");
}
