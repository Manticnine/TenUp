...
bool CTupStake::InitFromTxIn(const CTxIn& txin)
{
    if (txin.IsZerocoinSpend())
        return error("%s: unable to initialize CTupStake from zerocoin spend", __func__);
...
bool CTupStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CTupStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool CTupStake::GetTxOutFrom(CTxOut& out) const
{
...
CAmount CTupStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CTupStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal, const bool onlyP2PK)
{
...
CDataStream CTupStake::GetUniqueness() const
{
...
bool CTupStake::ContextCheck(int nHeight, uint32_t nTime)
{
