...
CzTUPTracker::CzTUPTracker(CWallet* parent)
{
    this->wallet = parent;
...
{
    bool iszTUPWalletInitialized = (nullptr != zTUPWallet);
    CMintMeta meta;
    ...
    if (!iszTUPWalletInitialized)
        zTUPWallet = new CzTUPWallet(wallet);
...
std::set<CMintMeta> CzTUPTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    ...
    if (fUpdateStatus) {
        ...
        LogPrint(BCLog::LEGACYZC, "%s: added %d dztup from DB\n", __func__, listDeterministicDB.size());
    }
