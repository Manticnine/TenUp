// Copyright (c) 2018-2019 The TenUp developers
...
CoinWitnessData* CzTUPTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzTUPTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
void CzTUPTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzTUPWallet* zTUPWallet)
{
    bool iszTUPWalletInitialized = (NULL != zTUPWallet);
    CMintMeta meta;
    ...
    if (! iszTUPWalletInitialized)
        zTUPWallet = new CzTUPWallet(strWalletFile);
    meta.isSeedCorrect = zTUPWallet->CheckSeed(dMint);
    if (! iszTUPWalletInitialized)
        delete zTUPWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
std::set<CMintMeta> CzTUPTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzTUPWallet* zTUPWallet = new CzTUPWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zTUPWallet);
        }
        delete zTUPWallet;
