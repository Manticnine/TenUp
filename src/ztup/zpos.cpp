...
bool CLegacyZTupStake::InitFromTxIn(const CTxIn& txin)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLegacyZTupStake from non zc-spend", __func__);
    ...
    *this = CLegacyZTupStake(spend);

    // Find the pindex with the accumulator checksum
    if (!GetIndexFrom())
        return error("%s : Failed to find the block index for ztup stake origin", __func__);

    // All good
    return true;
}

CLegacyZTupStake::CLegacyZTupStake(const libzerocoin::CoinSpend& spend)
{
...
CBlockIndex* CLegacyZTupStake::GetIndexFrom()
{
...
CAmount CLegacyZTupStake::GetValue() const
{
    return denom * COIN;
}

CDataStream CLegacyZTupStake::GetUniqueness() const
{
...
bool CLegacyZTupStake::ContextCheck(int nHeight, uint32_t nTime)
{
    const Consensus::Params& consensus = Params().GetConsensus();
    if (!consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_V2) || nHeight >= consensus.height_last_ZC_AccumCheckpoint)
        return error("%s : zTUP stake block: height %d outside range", __func__, nHeight);
