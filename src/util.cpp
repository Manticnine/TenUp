...
const char * const TUPX_CONF_FILENAME = "tenup.conf";
const char * const TUPX_PID_FILENAME = "tenup.pid";
const char * const TUPX_MASTERNODE_CONF_FILENAME = "masternode.conf";
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\TUPXParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\TUPXParams
    // Mac: ~/Library/Application Support/TUPXParams
    // Unix: ~/.tenup-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "TUPXParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "TUPXParams";
#else
    // Unix
    return pathRet / ".tenup-params";
#endif
#endif
}
...
{
    fs::path pathConfigFile(GetArg("-conf", TUPX_CONF_FILENAME));
    return AbsPathForConfigVal(pathConfigFile, false);
}

fs::path GetMasternodeConfigFile()
{
    fs::path pathConfigFile(GetArg("-mnconf", TUPX_MASTERNODE_CONF_FILENAME));
...
fs::path GetPidFile()
{
    fs::path pathPidFile(GetArg("-pid", TUPX_PID_FILENAME));
