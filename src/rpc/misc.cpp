...
UniValue getinfo(const JSONRPCRequest& request)
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        if (mapZerocoinSupply.empty())
            ztupObj.push_back(Pair(std::to_string(denom), ValueFromAmount(0)));
        else
            ztupObj.push_back(Pair(std::to_string(denom), ValueFromAmount(mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    ztupObj.push_back(Pair("total", ValueFromAmount(GetZerocoinSupply())));
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "  \"isstaking\" : true|false,       (boolean) If the address is a staking address for TenUp cold staking -only if is standard address-\n"
