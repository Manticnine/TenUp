...
UniValue createrawtransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw std::runtime_error(
            ...
            "    {\n"
            "      \"address\": x.xxx   (numeric, required) The key is the tenup address, the value is the tenup amount\n"
            "      ,...\n"
            "    }\n"
...
UniValue createrawzerocoinstake(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            ...
            "\nCreates raw zTUP coinstakes (without MN output). Only for regtest\n" +
            HelpRequiringPassphrase() + "\n"
    ...
    // create the zerocoinmint output (one spent denom + three 1-zTUP denom)
    ...
    if (!pwalletMain->CreateZTUPOutPut(staked_denom, vOutMint[1], dMint))
        throw JSONRPCError(RPC_WALLET_ERROR, "failed to create new ztup output");

    for (int i=2; i<5; i++) {
        if (!pwalletMain->CreateZTUPOutPut(libzerocoin::ZQ_ONE, vOutMint[i], dMint))
            throw JSONRPCError(RPC_WALLET_ERROR, "failed to create new ztup output");
    }
    ...
    CTxIn newTxIn;
    // Use v2 spends for input (with v3 ztup staking is disabled)
