...
#include "qt/tenup/splash.h"
#include "qt/tenup/welcomecontentwidget.h"
...
public:
    ...
    /// Get window identifier of QMainWindow (TUPXGUI)
...
{
    window = new TUPXGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &TUPXGUI::detectShutdown);
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &TUPXGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(TUPXGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(TUPXGUI::DEFAULT_WALLET);
        ...
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &TUPXGUI::handlePaymentRequest);
        connect(window, &TUPXGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. TenUp can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
