...
#include "qt/tenup/privacywidget.h"
#include "qt/tenup/forms/ui_privacywidget.h"
#include "qt/tenup/qtutils.h"
#include "guiutil.h"
#include "qt/tenup/denomgenerationdialog.h"
#include "qt/tenup/txviewholder.h"
...
PrivacyWidget::PrivacyWidget(TENUPGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zTUP anonymizes your TUP by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zTUP or convert back to TUP"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 TUP ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zTUP");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zTUP");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zTUP");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zTUP");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zTUP");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zTUP");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zTUP");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zTUP");
    setCssProperty(ui->labelValueDenom5000, "text-body2");

    ui->layoutDenom->setVisible(false);

    // List
    ui->labelListHistory->setText(tr("Last zTUP Movements"));
    ...
    // Only Convert to TUP enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzTUP->setTitleClassAndText("btn-title-grey", "Total 0 zTUP");
    ui->btnTotalzTUP->setSubTitleClassAndText("text-subtitle", "Show denominations of zTUP owned.");
    ui->btnTotalzTUP->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select TUP outputs to mint into zTUP.");
    ...
    ui->btnResetZerocoin->setTitleClassAndText("btn-title-grey", "Reset Spent zTUP");
    ui->btnResetZerocoin->setSubTitleClassAndText("text-subtitle", "Reset zerocoin database.");

    connect(ui->btnTotalzTUP, SIGNAL(clicked()), this, SLOT(onTotalZtupClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zTUP");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of TUP to mint into zTUP"));
    }else{
        btnText = tr("Convert back to TUP");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zTUP to convert back into TUP"));
    }
...
void PrivacyWidget::onTotalZtupClicked(){
    bool isVisible = ui->layoutDenom->isVisible();
    if(!isVisible){
        ui->layoutDenom->setVisible(true);
        ui->btnTotalzTUP->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzTUP->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zTUP is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zTUP, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zTUP").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zTUP minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZtup(
            value,
            selectedMints,
            mintChange,
            minimizeChange,
        ...
        // Spend succeed
        inform(tr("zTUP converted back to TUP"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::onCoinControlClicked(){
    if(ui->pushRight->isChecked()) {
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any TUP to select."));
        }
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    ui->btnTotalzTUP->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
