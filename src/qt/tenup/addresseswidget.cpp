...
#include "qt/tenup/addresseswidget.h"
#include "qt/tenup/forms/ui_addresseswidget.h"
#include "qt/tenup/addresslabelrow.h"
#include "qt/tenup/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(TENUPGUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    // Address
    ui->labelAddress->setText(tr("Enter TENUP address"));
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress tupAdd = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(tupAdd.Get(), label.toUtf8().constData(),
                tupAdd.IsStakingAddress() ? AddressBook::AddressBookPurpose::COLD_STAKING_SEND : AddressBook::AddressBookPurpose::SEND)
                ) {
