...
#include "qt/tenup/topbar.h"
#include "qt/tenup/forms/ui_topbar.h"
#include <QPixmap>
#include "qt/tenup/lockunlock.h"
#include "qt/tenup/qtutils.h"
#include "qt/tenup/receivedialog.h"
...
TopBar::TopBar(TENUPGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopTup, ui->labelAmountTopzTup}, "amount-small-topbar");
    setCssProperty({ui->labelAmountTup, ui->labelAmountzTup}, "amount-topbar");
    setCssProperty({ui->labelPendingTup, ui->labelPendingzTup, ui->labelImmatureTup, ui->labelImmaturezTup}, "amount-small-topbar");
...
void TopBar::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("TENUP")
    updateDisplayUnit();
...
void TopBar::updateBalances(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance,
    ...
    // TUP Total
    CAmount tupAvailableBalance = balance;
    // zTUP Balance
    CAmount matureZerocoinBalance = zerocoinBalance - unconfirmedZerocoinBalance - immatureZerocoinBalance;

    // Set
    QString totalTup = GUIUtil::formatBalance(tupAvailableBalance, nDisplayUnit);
    QString totalzTup = GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true);
    // Top
    ui->labelAmountTopTup->setText(totalTup);
    ui->labelAmountTopzTup->setText(totalzTup);

    // Expanded
    ui->labelAmountTup->setText(totalTup);
    ui->labelAmountzTup->setText(totalzTup);

    ui->labelPendingTup->setText(GUIUtil::formatBalance(unconfirmedBalance, nDisplayUnit));
    ui->labelPendingzTup->setText(GUIUtil::formatBalance(unconfirmedZerocoinBalance, nDisplayUnit, true));

    ui->labelImmatureTup->setText(GUIUtil::formatBalance(immatureBalance, nDisplayUnit));
    ui->labelImmaturezTup->setText(GUIUtil::formatBalance(immatureZerocoinBalance, nDisplayUnit, true));
}
