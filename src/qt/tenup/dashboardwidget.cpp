...
#include "qt/tenup/dashboardwidget.h"
#include "qt/tenup/forms/ui_dashboardwidget.h"
#include "qt/tenup/sendconfirmdialog.h"
#include "qt/tenup/txrow.h"
...
DashboardWidget::DashboardWidget(TENUPGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of TUP and zTUP staked."));
    setCssSubtitleScreen(ui->labelMessage);
    setCssProperty(ui->labelSquareTup, "square-chart-tup");
    setCssProperty(ui->labelSquarezTup, "square-chart-ztup");
    setCssProperty(ui->labelTup, "text-chart-tup");
    setCssProperty(ui->labelZtup, "text-chart-ztup");
    ...
    ui->labelAmountZtup->setText("0 zTUP");
    ui->labelAmountTup->setText("0 TUP");
    setCssProperty(ui->labelAmountTup, "text-stake-tup-disable");
    setCssProperty(ui->labelAmountZtup, "text-stake-ztup-disable");
    ...
    ui->btnHowTo->setText(tr("How to get TUP or zTUP"));
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy() {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isTup = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZTUP;
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
    ...
    chartData->amountsByCache = getAmountBy(); // pair TUP, zTUP
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            tup = (pair.first != 0) ? pair.first / 100000000 : 0;
            ztup = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalTup += pair.first;
            chartData->totalZtup += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesTup.append(tup);
        chartData->valueszTup.append(ztup);

        int max = std::max(tup, ztup);
...
void DashboardWidget::onChartRefreshed() {
    ...
    // init sets
    set0 = new QBarSet("TUP");
    set1 = new QBarSet("zTUP");
    ...
    set0->append(chartData->valuesTup);
    set1->append(chartData->valueszTup);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalTup > 0 || chartData->totalZtup > 0) {
        setCssProperty(ui->labelAmountTup, "text-stake-tup");
        setCssProperty(ui->labelAmountZtup, "text-stake-ztup");
    } else {
        setCssProperty(ui->labelAmountTup, "text-stake-tup-disable");
        setCssProperty(ui->labelAmountZtup, "text-stake-ztup-disable");
    }
    forceUpdateStyle({ui->labelAmountTup, ui->labelAmountZtup});
    ui->labelAmountTup->setText(GUIUtil::formatBalance(chartData->totalTup, nDisplayUnit));
    ui->labelAmountZtup->setText(GUIUtil::formatBalance(chartData->totalZtup, nDisplayUnit, true));
