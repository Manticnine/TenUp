...
#include "qt/tenup/forms/ui_snackbar.h"
#include "qt/tenup/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(TUPXGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &TUPXGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
