...
#include "qt/tenup/send.h"
#include "qt/tenup/forms/ui_send.h"
#include "qt/tenup/addnewcontactdialog.h"
#include "qt/tenup/qtutils.h"
#include "qt/tenup/sendchangeaddressdialog.h"
#include "qt/tenup/optionbutton.h"
#include "qt/tenup/sendconfirmdialog.h"
#include "qt/tenup/myaddressrow.h"
#include "qt/tenup/guitransactionsutils.h"
...
SendWidget::SendWidget(TENUPGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Button Group */
    ui->pushLeft->setText("TUP");
    setCssProperty(ui->pushLeft, "btn-check-left");
    ui->pushLeft->setChecked(true);
    ui->pushRight->setText("zTUP");
    setCssProperty(ui->pushRight, "btn-check-right");

    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (TUP) or private coins (zTUP)"));
    ...
    /* Address */
    ui->labelSubtitleAddress->setText(tr("TENUP address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 TUP");
    ...
    setCssProperty(coinIcon, "coin-icon-tup");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onTUPSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onTUPSelected(false);});
...
void SendWidget::refreshAmounts() {
    ...
    bool isZtup = ui->pushRight->isChecked();
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();

    ui->labelAmountSend->setText(GUIUtil::formatBalance(total, nDisplayUnit, isZtup));
    ...
    if (CoinControlDialog::coinControl->HasSelected()){
        ...
        // Wallet's balance
        totalAmount = (isZtup ? walletModel->getZerocoinBalance() : walletModel->getBalance()) - total;
...
void SendWidget::onSendClicked(){
    ...
    bool sendTup = ui->pushLeft->isChecked();

    // request unlock only if was locked or unlocked for mixing:
    // this way we let users unlock by walletpassphrase or by menu
    // and make many transactions while unlocking through this dialog
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendTup ? AskPassphraseDialog::Context::Send_TUP : AskPassphraseDialog::Context::Send_zTUP, true)){
        // Unlock wallet was cancelled
        inform(tr("Cannot send, wallet locked"));
        return;
    }

    if((sendTup) ? send(recipients) : sendZtup(recipients)) {
...
bool SendWidget::sendZtup(QList<SendCoinsRecipient> recipients){
    if (!walletModel || !walletModel->getOptionsModel())
        return false;

    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("zTUP is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    if (walletModel->sendZtup(
        ...
        inform(tr("zTUP transaction sent!"));
        ...
        if (receipt.GetStatus() == ZTUP_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zTUP require a security level of 100 to successfully spend.");
        } else {
        ...
        emit message("zTUP transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isTUP){
        ...
        if (walletModel->getZerocoinBalance() > 0) {
            ...
            zTupControl->exec();
            ui->btnCoinControl->setActive(!ZTupControlDialog::setSelectedMints.empty());
            zTupControl->deleteLater();
        } else {
            inform(tr("You don't have any zTUP in your balance to select."));
        }
    }
}
...
void SendWidget::onTUPSelected(bool _isTUP){
    isTUP = _isTUP;
    setCssProperty(coinIcon, _isTUP ? "coin-icon-tup" : "coin-icon-ztup");
    refreshView();
...
void SendWidget::onContactMultiClicked(){
    if(focusedEntry) {
        ...
        CBitcoinAddress tupAdd = CBitcoinAddress(address.toStdString());
        if (walletModel->isMine(tupAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(tupAdd.Get(), dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
