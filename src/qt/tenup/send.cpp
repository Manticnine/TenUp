...
SendWidget::SendWidget(TUPXGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        CTxDestination tupAdd = DecodeDestination(address.toStdString());
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(tupAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
