...
class ReorgStakeTest(TenUpTestFramework):
    ...
    def check_money_supply(self, expected_tup, expected_ztup):
        g_info = [self.nodes[i].getinfo() for i in range(self.num_nodes)]
        # verify that nodes have the expected TUP and zTUP supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_tup))
            for denom in node['zTUPsupply']:
                assert_equal(node['zTUPsupply'][denom], DecimalAmt(expected_ztup[denom]))
    ...
    def run_test(self):
        ...
        # Check TUP and zTUP supply at the beginning
        # ------------------------------------------
        # zTUP supply: 2 coins for each denomination
        expected_ztup_supply = {
            "1": 2,
            "5": 10,
            "10": 20,
            "50": 100,
        ...
        # TUP supply: block rewards minus burned fees for minting
        expected_money_supply = 250.0 * 330 - 16 * 0.01
        self.check_money_supply(expected_money_supply, expected_ztup_supply)
        ...
        # Verify that TUP and zTUP supplies were properly updated after the spends and reorgs
        self.log.info("Check TUP and zTUP supply...")
        ...
        expected_ztup_supply[str(spent_coin_0)] -= spent_coin_0
        expected_ztup_supply[str(spent_coin_1)] -= spent_coin_1
        expected_ztup_supply["total"] -= (spent_coin_0 + spent_coin_1)
        self.check_money_supply(expected_money_supply, expected_ztup_supply)
